...
export default function (eleventyConfig) {
	...
	eleventyConfig.addCollection('reads', (collectionApi) => {
		...
		allSubjects.reduce((postsByProgress, bookPost) => {
			postsByProgress[bookPost.progress ?? 'finished'].push(bookPost);
			return postsByProgress;
		}, allSubjects);

		// Show the most recently updated books first within each progress group
		const byNewest = (a, b) =>
			new Date(b.lastUpdated) - new Date(a.lastUpdated);
		allSubjects.want.sort(byNewest);
		allSubjects.started.sort(byNewest);
		allSubjects.finished.sort(byNewest);
		allSubjects.stopped.sort(byNewest);
